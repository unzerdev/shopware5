...
{block name='frontend_checkout_finish_items'}
    {block name="frontend_checkout_finish_unzer_payment_paylater-invoice_bank_data"}
        <div class="unzer-payment--info-panel">
            <div class="panel has--border is--wide is--rounded">
                {block name="frontend_checkout_finish_unzer_payment_Invoice_bank_data_title"}
                    <div class="panel--title is--underline payment--title">
                        {s name="title"}{/s}
                    </div>
                {/block}

                {block name="frontend_checkout_finish_unzer_payment_Invoice_bank_body"}
                    <div class="panel--body is--wide payment--content">

                        {block name="frontend_checkout_finish_unzer_payment_Invoice_bank_body_message"}
                            <div>
                                {s name="message"}{/s}
                            </div>
                        {/block}

                        {block name="frontend_checkout_finish_unzer_payment_Invoice_bank_body_table"}
